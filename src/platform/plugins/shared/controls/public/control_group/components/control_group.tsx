    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (target.tagName.toLowerCase() === 'li' || target.closest('li[role="option"]') !== null) {
        ...
        if (liElement && portalElement) {
          createFilterAnimation(liElement, elementRectInitial);
          ...
          <EuiPopover
            button={
              <EuiButton
                buttonRef={buttonRef}
                size="m"
                disabled={firstRender || (!hasUnappliedSelections && !firstRender)}
                iconSize="m"
                color={'success'}
                iconType={'check'}
                data-test-subj="controlGroup--applyFiltersButton"
            ...
            panelStyle={{ backgroundColor: '#4D84DC', color: 'white' }}
